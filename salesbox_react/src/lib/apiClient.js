...
export const API_URL = route ? `${protocol}://${host}/${route}` : `${protocol}://${host}`;
...
class API {
  ...
  constructor() {
    ...
    this.delete = this.createMethod('delete', this.axios);
    this.put = this.createMethod('put', this.axios);
  }
  ...
  createMethod(method: MethodT, api: APIT) {
    return async ({ resource, data = {}, query = {}, schema, options = {} }: RequestT) => {
      try {
        ...
        if (resource) {
          if (resource === ipInfoEP || resource?.includes(redirectFortnox) || resource?.includes(apiGeSessionIdFortnox)) {
        ...
        if(error?.response?.data?.errorMessage === 'EXISTED' && error?.response?.data?.additionInfo != null) {
          throw error?.response?.data;
        }
